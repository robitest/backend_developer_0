...
<main class="container my-3 d-flex flex-column flex-grow-1 vh-100">
    ...
    <form class="row g-3 mt-3" method="POST" action="/prices">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control <?= isset($errors['tip_filma']) ? 'is-invalid' : '' ?>" id="tip_filma" name="tip_filma" placeholder="Tip Filma">
            <div class="invalid-feedback"><?= $errors['tip_filma'] ?? '' ?></div>
        </div>
        <div class="col-md-4">
            <label for="cijena" class="form-label">Cijena</label>
            <input type="number" step="0.01" class="form-control <?= isset($errors['cijena']) ? 'is-invalid' : '' ?>" id="cijena" name="cijena" placeholder="Cijena">
            <div class="invalid-feedback"><?= $errors['cijena'] ?? '' ?></div>
        </div>
        <div class="col-md-4">
            <label for="zakasnina_po_danu" class="form-label">Zakasnina</label>
            <input type="number" step="0.01" class="form-control <?= isset($errors['zakasnina_po_danu']) ? 'is-invalid' : '' ?>" id="zakasnina_po_danu" name="zakasnina_po_danu" placeholder="Zakasnina">
